...
def main():
    ...
    print("=" * 65)
    print("Mini GPT Interactive Instruction-to-Code Generator")
    print("Enter an instruction (e.g. 'Write a function to square a number')")
    ...
    while True:
        ...
        # Format the prompt according to the training dataset structure
        formatted_prompt = f"Instruction: {prompt}\n\nCode:\n"
        
        # 4. Encode the formatted prompt into token IDs
        # input_ids shape: (batch_size=1, seq_len)
        input_ids = torch.tensor([tokenizer.encode(formatted_prompt)], dtype=torch.long)
        
        # Disable gradient calculations to speed up generation and save memory
        with torch.no_grad():
            # 9. Repeat generation for 150 characters (extended from 100 to allow complete function body)
            for _ in range(150):
                ...
                # --- Stopping Heuristics ---
                # Decode the text generated so far and extract the suffix (characters appended after the formatted prompt)
                current_text = tokenizer.decode(input_ids[0].tolist())
                generated_suffix = current_text[len(formatted_prompt):]
                
                # Stop if the model generates the "<END>" token sequence
                if "<END>" in generated_suffix:
                    # Truncate at the start of "<END>"
                    cutoff_idx = len(formatted_prompt) + generated_suffix.index("<END>")
        ...
        # 11. Extract and print only the generated Python code block cleanly
        completion = decoded_text[len(formatted_prompt):]
        print("\nGenerated Program:")
        print("-" * 45)
